...
GLfloat vetor_cor_azul[] = {0, 0, 1, 0};
float brilho_esfera = 50;   // Specular exponent for the spheres.
float cores_esferas_diffuse_ambient[6][4] = {      // The ambient/diffuse colors of the six spheres
    {0.5, 0.0, 0.0, 1.0},         // Red
...
float specular_sphere[4] = {1, 1, 1, 1.0};

// Lighting values
float luz_ambiente_global[4] = {0.5, 0.5, 0.5, 1.0};
float light_0_ambient[4] = {0.0, 0.0, 0.0, 1.0};
float light_0_diffuse[4] = {0.0, 0.0, 0.0, 1.0};
float light_0_specular[4] = {1.0, 0.0, 0.0, 0.0};
...
GLfloat angulo_rotacao_cubos = 0;
const GLfloat aumento_angulo_rotacao_cubos = 1;
...
//  glTranslatef(7.0, 0.0, 0.0);               // Translate rotation center to origin
  glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, light_0_specular);   // Make sphere glow (emissive)
...
//  for (i = 0; i < 6; i++) {
//    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, cores_esferas_diffuse_ambient[i]);
//    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular_sphere);
//    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brilho_esfera);
//    glPushMatrix();
...
/** DESENHA A ESFERA. ESSE É BOM, FUNCIONA.*/
//  int i = 1;
//  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, cores_esferas_diffuse_ambient[i]);
//  glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular_sphere);
//  glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brilho_esfera);
...
//  glPopMatrix();
  ...
  for (int a = 0; a < 2; a++) {
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, vetor_cor_azul);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular_sphere);
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brilho_esfera);
    glPushMatrix();
    glTranslatef(3 * (a ? -1 : 1), 0.0, 0);
    glRotatef(angulo_rotacao_cubos, 0.0, 1.0, 0.0);   // Rotate each one another 60 degrees
    glutSolidCube(1);
    glPopMatrix();
  }
...
//  update(deltaSeconds);
  angulo_rotacao_cubos += aumento_angulo_rotacao_cubos;
